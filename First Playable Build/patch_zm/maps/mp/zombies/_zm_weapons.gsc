...
player_can_use_content( weapon )
{
    if ( isdefined( level.content_weapons ) )
    {
        if ( isdefined( level.content_weapons[weapon] ) )
            return self hasdlcavailable( level.content_weapons[weapon] ) || self hasdlcavailable( "seasonpass" );
    }
...
weapon_spawn_think()
{
    ...
    for (;;)
    {
        ...
        if ( isdefined( player.pers_upgrades_awarded["double_points"] ) && player.pers_upgrades_awarded["double_points"] )
        {
            cost = int( cost / 2 );
