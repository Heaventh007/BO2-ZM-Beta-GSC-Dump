...
pers_flopper_explode( attacker, origin )
{
    ...
    network_optimize_damage = 1;

    if ( !network_optimize_damage )
...
pers_magic_box_teddy_bear()
{
    ...
    m_bear pers_magic_box_set_teddy_location();
...
pers_magic_box_set_teddy_location()
{
    box = level.chests[level.chest_index];
    ...
    self.origin = ( v_origin[0], v_origin[1], v_origin[2] + height_offset );
...
pers_nube_should_we_give_raygun( player_has_weapon, player, weapon_buy )
{
    ...
    if ( !player_has_weapon )
    {
        if ( player hasweapon( "ray_gun_zm" ) )
            player_has_weapon = 1;
        else if ( player hasweapon( "ray_gun_upgraded_zm" ) )
            player_has_weapon = 1;
    }
