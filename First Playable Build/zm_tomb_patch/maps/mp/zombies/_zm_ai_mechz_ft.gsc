...
mechz_flamethrower_aim()
{
    ...
    while ( true )
    {
        if ( !isdefined( self.favoriteenemy ) || !is_player_valid( self.favoriteenemy, 1, 1 ) || distancesquared( self.origin, self.favoriteenemy.origin ) > 90000 )
        {
            self notify( "stop_ft" );
            return;
        }

        aim_anim = mechz_get_aim_anim( "zm_flamethrower", self.favoriteenemy.origin );
    ...
    while ( true )
    {
        ...
        while ( isdefined( self.firing ) && self.firing )
        {
            ...
            for ( i = 0; i < players.size; i++ )
            {
                ...
                {
                    if ( players[i] istouching( self.flamethrower_trigger ) )
            ...
            for ( i = 0; i < zombies.size; i++ )
            {
                ...
                if ( zombies[i] istouching( self.flamethrower_trigger ) )
                {
                    zombies[i].on_fire = 1;
...
#/
    ...
    if ( n_dist_sq < 16641 || n_dist_sq > 90000 )
    {
/#
        if ( getdvarint( #"_id_E7121222" ) > 1 )
...
#/
    ...
    if ( randomint( 100 ) < level.mechz_ft_sweep_chance )
    {
    ...
    else
    {
        ...
        self animscripted( self.origin, self.angles, "zm_flamethrower_aim_end" );
        self maps\mp\animscripts\zm_shared::donotetracks( "flamethrower_anim" );
    }
