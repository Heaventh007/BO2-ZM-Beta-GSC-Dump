...
#/
    ...
    registerclientfield( "toplayer", "dive2nuke_visionset", 1, 1, "int", ::zombie_dive2nuke_visionset, 1, 1 );
    registerclientfield( "toplayer", "deadshot_perk", 1, 1, "int", ::player_deadshot_perk_handler, 1, 1 );
        ...
        while ( true )
        {
            if ( !self islocalplayer() || isspectating( localclientnum, 1 ) || localclientnum != self getlocalclientnumber() )
            {
                if ( !is_true( self.eyes_spawned ) )
                {
                    ...
                    self mapshaderconstant( localclientnum, 0, "scriptVector2", 0, 8 );
                    self.eyes_spawned = 1;
                }
            }
...
zombie_dive2nuke_visionset( localclientnum, oldval, newval, bnewent, binitialsnap, fieldname, bwasdemojump )
{
    self endon( "disconnect" );
...
#/
        ...
        self thread zombie_vision_set_apply( "zombie_cosmodrome_diveToNuke", 11, time_to_apply_vision, localclientnum );
        wait 0.5;
        self thread zombie_vision_set_remove( "zombie_cosmodrome_diveToNuke", time_to_remove_vision, localclientnum );
    }
}
