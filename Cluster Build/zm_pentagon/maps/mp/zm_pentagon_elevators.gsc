...
init()
{
    ...
    elevator1.travel_down = elevator1.origin - vectorscale( ( 0, 0, 1 ), 201.0 );
    elevator2.cost = 250;
    elevator2.station = "elevator2_up";
    elevator2.called = 0;
    elevator2.active = 0;
    elevator2.travel_up = elevator2.origin;
    elevator2.travel_down = elevator2.origin - vectorscale( ( 0, 0, 1 ), 320.0 );
...
elevator_buy_think( elevator )
{
    ...
    while ( true )
    {
        ...
        {
            elevator.active = 1;
            who maps\mp\zombies\_zm_score::minus_to_player_score( elevator.cost );
        ...
        {
            play_sound_at_pos( "no_purchase", self.origin );
            who maps\mp\zombies\_zm_audio::create_and_play_dialog( "general", "no_money", undefined, 1 );
        }
...
elevator_move_to( elevator )
{
    ...
    {
        elevator thread move_zombies_elevator( 0 );
    ...
    {
        elevator thread move_zombies_elevator( 1 );
...
move_zombies_elevator( going_up )
{
    ...
    for ( i = 0; i < players.size; i++ )
    {
        ...
        if ( players[i] istouching( check_trig ) )
        {
            ...
            self.elevator_players[self.elevator_players.size] = players[i];
            ...
            if ( self.targetname == "elevator1" )
                setclientsysstate( "levelNotify", "ele2", players[i] );
        }
    }

    if ( isdefined( current_floor ) && going_up == 0 )
        next_floor = current_floor + 1;
    else if ( isdefined( current_floor ) && going_up == 1 )
        next_floor = current_floor - 1;
    ...
    for ( i = 0; i < players.size; i++ )
    {
        ...
        if ( isdefined( current_floor ) && players[i].floor == current_floor && ( players[i] maps\mp\zombies\_zm_laststand::player_is_in_laststand() || players[i].sessionstate == "spectator" ) && !players[i] istouching( check_trig ) )
        {
    ...
    for ( i = 0; i < zombies.size; i++ )
    {
        ...
        else
        {
            ...
            if ( pos_num_hidden <= 5 && zombies[i].animname != "quad_zombie" )
            {
            ...
            if ( isdefined( current_floor ) && current_floor == 2 && flag( "power_on" ) )
            {
                zombies[i] thread send_zombies_out( level.portal_mid );
                continue;
            }

            if ( isdefined( current_floor ) && current_floor == 3 && flag( "power_on" ) )
            {
...
laststand_elev_zombies_away( current_floor, next_floor )
{
    ...
    for ( i = 0; i < zombies.size; i++ )
    {
        ...
        if ( isdefined( zombies[i].floor ) && zombies[i].floor == self.floor )
        {
            ...
            if ( zombies[i].floor == 2 && flag( "power_on" ) )
            {
                zombies[i] thread send_zombies_out( level.portal_mid );
                continue;
            }

            if ( zombies[i].floor == 3 && flag( "power_on" ) )
            {
                zombies[i] thread send_zombies_out( level.portal_power );
                continue;
            }
...
open_elev_doors()
{
    if ( isdefined( self.elevator_players ) )
    {
        ...
        for ( i = 0; i < self.elevator_players.size; i++ )
        {
            ...
            arrayremovevalue( self.elevator_players, self.elevator_players[i] );
        }
    }
...
redirect_zombies( destination )
{
    ...
    if ( isdefined( location ) )
    {
        ...
        poi create_zombie_point_of_interest( undefined, 25, 0, 1 );
...
#/
        players[i] thread maps\mp\zombies\_zm::store_crumb( players[i].origin );
    }
}
...
elevator1_3d_audio()
{
    while ( true )
    {
        ...
        ent = spawn( "script_origin", self.origin + vectorscale( ( 0, 0, 1 ), 30.0 ) );
        ent linkto( self );
