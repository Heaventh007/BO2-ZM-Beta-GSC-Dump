...
heli_think( owner, startnode, heli_team, missilesenabled, protectlocation, hardpointtype, armored, killstreak_id )
{
    ...
    chopper thread maps\mp\_heatseekingmissile::missiletarget_proximitydetonateincomingmissile( "crashing", "leaving" );
...
heli_targeting( missilesenabled, hardpointtype )
{
    ...
    for (;;)
    {
        ...
        {
            if ( self cantargetdog_turret( tank ) )
                targets[targets.size] = tank;

            if ( missilesenabled && self cantargetdog_missile( tank ) )
                targetsmissile[targetsmissile.size] = tank;
        }
...
cantargetdog_turret( dog )
{
    ...
    if ( dog.lasthit != 0 && dog.type != "tank_drone" )
        return 0;
    ...
    for (;;)
    {
        ...
        if ( self.damagetaken > self.maxhealth && !isdefined( self.xpgiven ) && ( !isdefined( self.owner ) || attacker != self.owner ) )
        {
            ...
            if ( isdefined( event ) )
            {
                ...
                {
                    maps\mp\_challenges::destroyedhelicopter( attacker, weapon, type );
