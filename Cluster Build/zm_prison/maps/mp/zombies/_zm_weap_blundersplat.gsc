    ...
    while ( true )
    {
        ...
        if ( weaponname == "blundersplat_bullet_zm" )
        {
            if ( !isdefined( self.titus_tagged ) )
            {
                ...
                {
                    if ( e_grenade.model == "t6_wpn_projectile_titus" )
                    {
...
_titus_locate_target( is_upgraded )
{
    ...
    if ( is_upgraded )
        ...
        n_fuse_timer = randomfloatrange( 1, 3.5 );
...
_titus_reset_grenade_fuse( n_fuse_timer )
{
...
gib_on_blundergat_damage( point, weaponname )
{
    new_gib_ref = [];
