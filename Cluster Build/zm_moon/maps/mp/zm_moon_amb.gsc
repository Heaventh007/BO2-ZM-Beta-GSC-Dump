...
play_radio_eastereggs()
{
    ...
    while ( true )
    {
        self thread maps\mp\zombies\_zm_sidequests::fake_use( "radio_activate" );
...
{
    flag_wait( "power_on" );
    self thread maps\mp\zombies\_zm_sidequests::fake_use( "bit_hit", ::waitfor_override );
...
intro_vox_or_skit()
{
    ...
    wait 3;
    players = get_players();
    players[randomintrange( 0, players.size )] thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "general", "start" );
}
...
poweron_vox()
{
    wait 3;
    flag_wait( "power_on" );
    level thread maps\mp\zm_moon_amb::play_mooncomp_vox( "vox_mcomp_power" );
}
...
do_player_playvox_custom( prefix, index, sound_to_play, waittime, category, type, override )
{
    ...
    if ( is_true( self.in_low_gravity ) && !self maps\mp\zombies\_zm_equip_gasmask::gasmask_active() )
        return;
...
play_futz_or_not_moonvox( prefix, sound_to_play )
{
    ...
    for ( i = 0; i < players.size; i++ )
    {
        if ( self maps\mp\zombies\_zm_equipment::is_equipment_active( "equip_gasmask_zm" ) )
        {
            ...
            else if ( players[i] maps\mp\zombies\_zm_equipment::is_equipment_active( "equip_gasmask_zm" ) )
...
music_egg()
{
    ...
    self thread maps\mp\zombies\_zm_sidequests::fake_use( "main_music_egg_hit", ::waitfor_override );
...
do_mooncomp_vox( alias )
{
    ...
    for ( i = 0; i < players.size; i++ )
    {
        if ( players[i] maps\mp\zombies\_zm_equipment::is_equipment_active( "equip_gasmask_zm" ) || is_true( players[i].in_low_gravity ) )
    ...
    while ( true )
    {
        ...
        for ( i = 0; i < zone.volumes.size; i++ )
        {
            for ( j = 0; j < players.size; j++ )
            {
                ...
                {
                    players[j] thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "general", "biodome" );
                    return;
                }
            }
        }
...
waitfor_first_player()
{
    ...
    who thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "general", "teleporter", undefined, undefined, 1 );
}
...
play_warning_vox()
{
    ...
    while ( true )
    {
        ...
        if ( is_true( self.in_low_gravity && self hasweapon( "equip_gasmask_zm" ) && !self maps\mp\zombies\_zm_equip_gasmask::gasmask_active() ) )
        {
            self stopsounds();
            wait 0.05;
            ...
            while ( self.in_low_gravity )
            {
                if ( self maps\mp\zombies\_zm_equip_gasmask::gasmask_active() )
                    break;
