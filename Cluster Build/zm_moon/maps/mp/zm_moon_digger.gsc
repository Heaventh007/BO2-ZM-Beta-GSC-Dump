...
{
    level thread maps\mp\zm_moon_amb::play_mooncomp_vox( "vox_mcomp_digger_start_", digger_name );
    wait 7;
    ...
    players = get_players();
    players[randomintrange( 0, players.size )] thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "digger", "incoming" );
}
...
digger_think_move()
{
    ...
    arm linkto( self );
    self linkto( tracks );
    tracks setclientfield( "digger_moving", 0 );
    arm setclientfield( "digger_arm_fx", 0 );
...
digger_arm_breach_logic( arm, blade_center, tracks )
{
    ...
    blade_center setclientfield( "digger_digging", 1 );
...
{
    arm setclientfield( "digger_arm_fx", 1 );
    tracks setclientfield( "digger_moving", 1 );
    ...
    if ( is_true( self.arm_lowered ) )
    {
        ...
        blade_center setclientfield( "digger_digging", 0 );
        blade_center linkto( arm );
        arm playsound( "evt_dig_arm_move" );
...
digger_think_panel( blocker_name, trig_name, start_flag, hacked_flag, hacked_before_breached_flag, breached_flag, blocker_func, digger_name )
{
    if ( isdefined( blocker_name ) )
    {
        ...
        {
            dmg_trig.origin = dmg_trig.origin + vectorscale( ( 0, -1, 0 ), 20.0 );
            blocker.origin = blocker.origin + vectorscale( ( 0, -1, 0 ), 20.0 );
        }
    ...
    struct = spawnstruct();
    struct.origin = trig.origin - vectorscale( ( 0, 0, 1 ), 12.0 );
    ...
    struct.custom_debug_color = vectorscale( ( 1, 0, 0 ), 255.0 );
    struct.radius = 64;
    struct.height = 64;
    ...
    trig thread maps\mp\zombies\_zm_equip_hacker::hide_hint_when_hackers_active();
...
set_hint_on_digger_trig( start_flag, hacked_flag, struct )
{
    while ( true )
    {
        if ( !flag( start_flag ) )
        {
            maps\mp\zombies\_zm_equip_hacker::deregister_hackable_struct( struct );
        ...
        if ( !flag( hacked_flag ) )
        {
            maps\mp\zombies\_zm_equip_hacker::register_pooled_hackable_struct( struct, ::digger_hack_func, ::digger_hack_qualifer );
...
digger_hack_func( hacker )
{
    ...
    hacker thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "digger", "hacked" );
...
{
    wait 4;
    level thread maps\mp\zm_moon_amb::play_mooncomp_vox( "vox_mcomp_digger_hacked_", digger );
}
    ...
    while ( true )
    {
        ...
        if ( name == digger_name )
        {
            ...
            for ( i = 0; i < zones.size; i++ )
            {
                ...
                level thread maps\mp\zm_moon_gravity::zone_breached( zone );
            }
...
{
    self thread maps\mp\zm_moon::insta_kill_player();
}
...
diggers_think_no_mans_land()
{
    ...
    {
        flag_wait( "enter_nml" );
        array_thread( diggers, ::diggers_visible, 0 );
        flag_waitopen( "enter_nml" );
        array_thread( diggers, ::diggers_visible, 1 );
    }
}
...
play_timer_vox( digger_name )
{
    ...
    while ( time_left > 0 )
    {
        ...
        {
            level thread maps\mp\zm_moon_amb::play_mooncomp_vox( "vox_mcomp_digger_time_60_", digger_name );
            played60sec = 1;
        }

        if ( time_left <= 30.0 && !played30sec )
        {
            level thread maps\mp\zm_moon_amb::play_mooncomp_vox( "vox_mcomp_digger_time_30_", digger_name );
            played30sec = 1;
        }
...
play_delayed_breach_vox( digger )
{
    ...
    playsoundatposition( "evt_breach_alarm", ( 0, 0, 0 ) );
    wait 2;
    level thread maps\mp\zm_moon_amb::play_mooncomp_vox( "vox_mcomp_digger_breach_", digger );
}
        ...
        for ( x = 0; x < _zones.size; x++ )
        {
            for ( j = 0; j < players.size; j++ )
            {
                if ( is_player_valid( players[j] ) && players[j] istouching( _zones[x] ) )
                    players[j] thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "digger", "breach" );
            }
        }
    }
}
...
digger_follow_path( body, reverse, arm )
{
    ...
    self setclientfield( "digger_moving", 1 );
    arm setclientfield( "digger_arm_fx", 1 );
    ...
    self setclientfield( "digger_moving", 0 );
    arm setclientfield( "digger_arm_fx", 0 );
    flag_clear( "digger_moving" );
