...
wall_move()
{
    ...
    exploder( 410 );
    self moveto( self.origin - vectorscale( ( 0, 0, 1 ), 4.0 ), 0.1, 0.01 );
    ...
    vec = vectornormalize( struct.origin + vectorscale( ( 0, 0, 1 ), 48.0 ) - self.origin );
...
reveal_music()
{
    ...
    level thread maps\mp\zombies\_zm_audio::change_zombie_music( "sam_reveal" );
    wait 40;
...
sam_reveal_richtofen_vox()
{
    ...
    for ( i = 0; i < players.size; i++ )
    {
        ...
        if ( index == 3 )
            players[i] thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "eggs", "quest4", undefined, 3, 1 );
    }
}
...
room_sweeper()
{
    ...
    level thread maps\mp\zombies\_zm_powerups::minigun_weapon_powerup( self, 90 );
...
{
    wait 5;
    player = maps\mp\zm_moon_sq::get_specific_player( 0 );
...
do_soul_swap( who )
{
    maps\mp\zm_moon_amb::player_4_override();

    if ( isdefined( who ) )
    {
        who setclientfieldtoplayer( "soul_swap", 1 );
        who maps\mp\zm_moon_sq::give_perk_reward();
    }

    wait 2.0;

    if ( isdefined( who ) )
        who setclientfieldtoplayer( "soul_swap", 0 );
...
play_sam_then_response_line()
{
    ...
    player[randomintrange( 0, player.size )] thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "eggs", "quest6", undefined, 13, 1 );
}
