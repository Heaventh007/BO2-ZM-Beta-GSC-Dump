...
plates()
{
    ...
    trig = spawn( "trigger_damage", ( plates[0].origin + plates[1].origin ) / 2 - vectorscale( ( 0, 0, 1 ), 100.0 ), 0, 64, 120 );
    ...
    while ( true )
    {
        ...
        {
            attacker thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "eggs", "quest5", undefined, randomintrange( 0, 2 ), 1 );
            break;
        }
    }
    ...
    level.teleport_target_trigger = spawn( "trigger_radius", plates[0].origin + vectorscale( ( 0, 0, -1 ), 70.0 ), 0, 125, 100 );
    ...
    maps\mp\zombies\_zm_weap_quantum_bomb::quantum_bomb_register_result( "ctvg", ::dud_func, 100, ::ctvg_validation );
    level._ctvg_pos = targs[0].origin;
    level waittill( "ctvg_validation" );
    maps\mp\zombies\_zm_weap_quantum_bomb::quantum_bomb_deregister_result( "ctvg" );
    players = get_players();
    players[randomintrange( 0, players.size )] thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "eggs", "quest5", undefined, randomintrange( 4, 6 ), 1 );
...
wire()
{
    ...
    who thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "eggs", "quest5", undefined, 7, 1 );
    ...
    who thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "eggs", "quest5", undefined, 8, 1 );
...
vg()
{
    ...
    who thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "eggs", "quest5", undefined, 9, 1 );
...
bhb_teleport_loc_check( grenade, model, info )
{
    if ( isdefined( level.teleport_target_trigger ) && grenade istouching( level.teleport_target_trigger ) )
    {
        ...
        spot setclientfield( "blackhole", 1 );
...
teleport_target( grenade, models )
{
    ...
    for ( i = 0; i < models.size; i++ )
        models[i] moveto( grenade.origin + vectorscale( ( 0, 0, 1 ), 50.0 ), time, time - 0.05 );
...
speak_charge_lines( lines )
{
    ...
    for ( i = 0; i < lines.size; i++ )
    {
        ...
        if ( is_player_valid( sound_ent ) && sound_ent maps\mp\zombies\_zm_equipment::is_equipment_active( "equip_gasmask_zm" ) )
...
do_bucket_fill( target )
{
    ...
    {
        level._charge_sound_ent thread maps\mp\zombies\_zm_sidequests::fake_use( "press", ::bucket_qualifier );
...
wrong_presser_thread()
{
    ...
    while ( true )
    {
        ...
        {
            level._charge_sound_ent thread maps\mp\zombies\_zm_sidequests::fake_use( "wrong_press", ::wrong_press_qualifier );
            level._charge_sound_ent waittill( "wrong_press", who );
            who thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "eggs", "quest5", undefined, 11, 1 );
        }
...
wrong_collector()
{
    ...
    while ( true )
    {
        self thread maps\mp\zombies\_zm_sidequests::fake_use( "wrong_collector", ::wrong_press_qualifier );
        self waittill( "wrong_collector", who );
        who thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "eggs", "quest5", undefined, 27, 1 );
        wait 1.0;
    }
}
...
charge_stage_logic()
{
    ...
    vg thread maps\mp\zombies\_zm_sidequests::fake_use( "collect", ::bucket_qualifier );
